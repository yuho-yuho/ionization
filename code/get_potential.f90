...
subroutine get_potential(iBlock)
  ...
  if (floor((tSimulation-dt)/DtAurora) /= &
     ...
     !elseif (useSDAM) then
        ...
        !--------------------------------------------------
        !! debug, yuhong, assigan Eave = 15, 10/03/2023
        if (iError == 0) then ! iproc ==21 .and. 
        ElectronAverageEnergy(1,1:3) = 20
        !ElectronAverageEnergy(6,1:3) = 0.5
        endif 
        !--------------------------------------------------
        if (iError /= 0) then
        ...
        do iLat=-1,nLats+2
           do iLon=-1,nLons+2
              if (ElectronAverageEnergy(iLon,iLat) < 0.0) then
                 ElectronAverageEnergy(iLon,iLat) = 1 
           ...
           enddo
        enddo
        !write(*,*) iLat, iLon ! yuhong

        call UA_GetEFlux(ElectronEnergyFlux, iError)
        !--------------------------------------------------
        !! debug, yuhong, assigan Eave = 15, 10/03/2023
        if (iError == 0) then ! iproc ==21 .and.
        ElectronEnergyFlux(1,1:3) = 1.0
        endif
        !--------------------------------------------------            
        if (iError /= 0) then
