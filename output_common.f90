...
subroutine output(dir, iBlock, iOutputType)
  ...
  case ('3DALL')
     ...
     nvars_to_write = 13+nSpeciesTotal+nSpecies+nIons &
          ...
          +2 + 1 & ! (TEC, int-JH, Jwd)
          +2 + 3 &
          +2 + 1 ! add 3D sigma, yuhong, 09/12/2023
          ! add ionization rate, yuhong, 10/09/2023
          ! check FAC input & output
          ! (AuroralBulkIonRate1, AuroralBulkIonRate2)
          ! add "ElectronAverageEnergy_plus"
  ...
  case ('2DMEL')
     ! yuhong, 02/11/2023, output in MAG coordinate 
     ! add inte-JH and potential, 02/11/2023
     nvars_to_write = 29 ! +2
  ...
  case ('1DALL')
     ...
     ! added FAC outputs for #Satellite, yuhong, 10/01/2021
     ! added JH, sigma and mag-lat/lon to #Satellite, yuhong, 04/08/2022
     ! added sigmaP & sigmaH to #Satellite, +2 yuhong, 11/06/2022
     ! added inte-JH for # Satellite, yuhong, 04/10/2022
     nvars_to_write = 13+nSpeciesTotal+nSpecies+nIons+3+2+1+2 !nSpecies+5
...
contains
  ...
  subroutine output_header
    ...
    if (cType(3:5) == "MEL") then
       ...
       ! add potential & int-JH for MAG, yuhong,02/11/2023
       !write(iOutputUnit_,"(I7,A1,a)") 30, " ", "Potential"
       !write(iOutputUnit_,"(I7,A1,a)") 31, " ", "Int-JH"
    ...
    if (cType(3:5) == "ALL" .or. cType(3:5) == "ION") then
       ...
       if (cType(3:5) == "ALL") then
          ...
          ! add FAC outputs for # Satellite, yuhong, 10/01/2021
          write(iOutputUnit_,"(I7,A1,a)") iOff+4, " ", "Gedy_fac" 
          write(iOutputUnit_,"(I7,A1,a)") iOff+5, " ", "Gedy_fac1" 
          write(iOutputUnit_,"(I7,A1,a)") iOff+6, " ", "Int-JH"
          ! add sigmaP & sigmaH for # Satellite, yuhong, 11/06/2022
          write(iOutputUnit_,"(I7,A1,a)") iOff+7, " ", "PedersenConductance"
          write(iOutputUnit_,"(I7,A1,a)") iOff+8, " ", "HallConductance"
          ...
          ! ------------------------- add 3D sigma ------------------------
          ! yuhong, 09/12/2023
          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "sigma_p"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "sigma_h"

          iOff = iOff + 2

          ! yuhong, 10/09/2023
          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "ionization_rate_o3p"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "ionization_rate_o2"
          write(iOutputUnit_,"(I7,A1,a)") iOff+3, " ", "ionization_rate_n2"

          iOff = iOff + 3

          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "BulkIonRate1"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "BulkIonRate2"
          
          iOff = iOff + 2

          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "AuroraEavePlus"
          iOff = iOff + 1
          ! ---------------------------------------------------------------
          ! add FAC readin & 2D interp, yuhong, 10/10/2021
          ! write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "fac_read_in"
          !write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "fac_2d_intep"
...
subroutine output_3dall(iBlock)
  ...
  use ModElectrodynamics, only: HallConductance, PedersenConductance, &
       Sigma_Pedersen, Sigma_Hall ! add 3D sigma, yuhong, 09/12/2023
  use ModSources, only: JouleHeating, AuroralIonRateS
  ! yuhong, 10/01/2021
  ! yuhong, add 'AuroralIonRateS', 10/13/2023
  !use ModGedy, only: poten_2d, poten1_2d
     ...
     do iLat=-1,nLats+2
        ...
        do iLon=-1,nLons+2
           ...
           write(iOutputUnit_)       &
                ...
                Sigma_Hall(iLon,iLat,iAlt), &
                ! yuhong, 10/09/2023
                AuroralIonRateS(iLon,iLat,iAlt,iO_3P_,iBlock), &
                AuroralIonRateS(iLon,iLat,iAlt,iO2_,iBlock), &
                AuroralIonRateS(iLon,iLat,iAlt,iN2_,iBlock), &
                AuroralBulkIonRate1(iiLon,iiLat,iiAlt), &
                AuroralBulkIonRate2(iiLon,iiLat,iiAlt), &
                ElectronAverageEnergy_plus(iLon,iLat)
...
subroutine output_1dall(iiLon, iiLat, iBlock, rLon, rLat, iUnit)
  ...
  ! qingyu, 02/07/2021
  ! yuhong, 10/01/2021, add FAC outputs
  ! yuhong, 04/08/2022, add JH, mag
  ! yuhong, 04/10/2022, add int-JH
  ! yuhong, 11/06/2022, add sigmaP & H
  integer, parameter :: nVars = 13+nSpeciesTotal+nSpecies+nIons+3+2+1+2 !nSpecies+5
...
!     enddo
        ...
        ! yuhong, 10/01/2021
        iOff = iOff + 1
        Tmp = Gedy_fac(0:nLons+1,0:nLats+1,iAlt)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = Gedy_fac1(0:nLons+1,0:nLats+1,iAlt)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
        
        ! yuhong, 04/08/2022                                                                                                                      
        iOff = iOff + 1
        !Tmp =  HeightIntegratedJH(0:nLons+1,0:nLats+1)
        !Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
        Vars(iOff) = HeightIntegratedJH(iiLon,iiLat)

        iOff = iOff + 1
        Tmp = PedersenConductance(0:nLons+1,0:nLats+1,iBlock) 
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = HallConductance(0:nLons+1,0:nLats+1,iBlock)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
        !------------------------------------------------------
        !!!! Not what we need
